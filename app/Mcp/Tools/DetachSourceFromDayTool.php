...
#[Name('detach-source-from-day')]
#[Description('Detach a source record from a day after preview and confirmation.')]
#[IsDestructive(false)]
class DetachSourceFromDayTool extends Tool
{
    /**
    ...
    public function handle(Request $request, McpMutationGuard $guard): ResponseFactory
    {
        ...
        return $guard->handle($request, $preview, function () use ($day, $source): array {
            $day->sources()->detach($source->id);

            return ['detached_source_id' => $source->id];
        });
    }
